...
DROGON_TEST(DatabaseInitialization)
{
    ...
    initialize(connection, "database");
    CHECK(connection.scalar("PRAGMA user_version") == 1);
    ...
    CHECK(connection.scalar("SELECT count(*) FROM sqlite_master WHERE type = 'index' AND name LIKE 'idx_%'") == 5);
...
DROGON_TEST(ConstraintsAndPreparedStatements)
{
    ...
    auto insert = connection.prepare("INSERT INTO users(display_name,email,password_hash,created_at,updated_at) VALUES(?,?,?,'2026-09-12T00:00:00Z','2026-09-12T00:00:00Z')");
